...
const EditMovieForm = ({ movieToEdit, onUpdateMovie, onClose }) => {
    ...
    const [actors, setActors] = useState(''); // Thêm trường Diễn viên cho đồng bộ
    const [description, setDescription] = useState('');
    const [releaseDate, setReleaseDate] = useState('');
    const [duration, setDuration] = useState('');
    const [movieStatus, setMovieStatus] = useState('COMING_SOON');
    ...
    useEffect(() => {
        if (movieToEdit) {
            ...
            setActors(movieToEdit.actors || ''); // Điền diễn viên cũ
            ...
            // Xử lý genres (Backend trả về mảng object -> chuyển thành Set ID)
    ...
    const handleSubmit = (e) => {
        ...
        const updatedMovie = {
            ...
            title, 
            director, 
            actors, // Gửi diễn viên lên server
            description, 
            releaseDate,
    ...
    return (
        // Sử dụng class 'wide-form' để kích hoạt giao diện 2 cột
                ...
                <div className="form-column right-col">
                    <div className="form-row-2">
                        ...
                        <div className="form-group">
                            ...
                            <input type="number" value={duration} onChange={(e) => setDuration(e.target.value)} placeholder="Ví dụ 120" min="1" required/> 
                        </div>
                    </div>
                            ...
                            <div className="genre-checkbox-group">
                                {allGenres.length > 0 ? allGenres.map((genre) => (
                                    <div key={genre.id} className="genre-checkbox-item">
                                        <input
                                            type="checkbox"
                                            id={`edit-genre-${genre.id}`} // ID khác với form Add để tránh xung đột
